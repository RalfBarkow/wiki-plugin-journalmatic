{
  description = "wiki-plugin-journalmatic dev shell with pack/install + local wiki runner + Stylelint";
      ...
      let
        ...
        pack_install_app = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            [ -f "''${WIKI_PLUGIN_DIR}/package.json" ] || { echo "ERROR: package.json not found in WIKI_PLUGIN_DIR: ''${WIKI_PLUGIN_DIR}" >&2; exit 1; }
            [ -d "''${WIKI_DIR}" ] || { echo "ERROR: WIKI_DIR does not exist: ''${WIKI_DIR}" >&2; exit 1; }

            echo "• Packing from: ''${WIKI_PLUGIN_DIR}"
            echo "• Installing into: ''${WIKI_DIR}"

            pushd "''${WIKI_DIR}" >/dev/null
            tgz="$(npm pack "''${WIKI_PLUGIN_DIR}" | tail -n1)"
            npm i "''${tgz}"
            echo "✔ Installed ''${tgz}"
            popd >/dev/null
          '';
        ...
        pack_install_watch_app = pkgs.writeShellApplication {
          ...
          text = ''
            set -euo pipefail
            WIKI_PLUGIN_DIR="''\${WIKI_PLUGIN_DIR:-$PWD}"
            echo "• Watching ''${WIKI_PLUGIN_DIR} for changes … (js,ts,elm,json,css,md)"
            exec watchexec \
              -w "''${WIKI_PLUGIN_DIR}" \
        ...
        stylelint_init_app = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            WIKI_PLUGIN_DIR="''\${WIKI_PLUGIN_DIR:-$PWD}"
            cd "''${WIKI_PLUGIN_DIR}"
            [ -f package.json ] || { echo "ERROR: no package.json in ''${WIKI_PLUGIN_DIR}" >&2; exit 1; }
...
EOF
        ...
        stylelint_watch_app = pkgs.writeShellApplication {
          ...
          text = ''
            set -euo pipefail
            WIKI_PLUGIN_DIR="''\${WIKI_PLUGIN_DIR:-$PWD}"
            echo "• Watching ''${WIKI_PLUGIN_DIR} for CSS/HTML/Vue changes …"
            exec watchexec \
              -w "''${WIKI_PLUGIN_DIR}" \
              -e css,html,vue \
              -- stylelint_run
      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "Current:"
            echo "  WIKI_PLUGIN_DIR=$WIKI_PLUGIN_DIR"
